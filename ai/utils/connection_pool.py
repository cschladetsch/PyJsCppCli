...
from ..utils.exceptions import NetworkError, RateLimitError, APIError
            ...
            try:
                ...
                if response.status >= 400:
                    ...
                    if response.status == 429:
                        ...
                        # Server error - retry
                        raise NetworkError(f"Server error: {response.status} - {error_text}")
                    else:
...
class AsyncAPIClient:
    ...
    def __init__(self, base_url: str, api_key: str, pool_config: Optional[Dict[str, Any]] = None):
        """
    ...
    async def post(self, endpoint: str, data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        async with self.pool.request('POST', url, json=data, headers=headers, **kwargs) as response:
    ...
    async def get(self, endpoint: str, params: Optional[Dict[str, Any]] = None, **kwargs) -> Dict[str, Any]:
        ...
        async with self.pool.request('GET', url, params=params, headers=headers, **kwargs) as response:
            return await response.json()
...
async def get_global_pool() -> ConnectionPool:
    ...
    if _global_pool is None:
        ...
        _global_pool = ConnectionPool(
            max_connections=config.api.max_retries * 2,
