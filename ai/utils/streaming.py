...
class StreamBuffer:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'duration': duration,
            'average_chunk_size': self.total_size / self.chunk_count if self.chunk_count > 0 else 0,
...
class StreamProcessor:
    ...
    async def process_stream(self, 
                           response: aiohttp.ClientResponse) -> AsyncGenerator[StreamEvent, None]:
        """
    ...
    async def _read_chunks(self, response: aiohttp.ClientResponse) -> AsyncGenerator[str, None]:
        """
        ...
        async for event in self.processor.process_stream(response):
            if event.event_type == StreamEventType.START:
                ...
                if self.progress_callback:
                    self.progress_callback({'status': 'started', 'metadata': event.metadata})
                ...
                if self.progress_callback:
                    self.progress_callback({'status': 'completed', 'stats': event.metadata})
...
async def stream_anthropic_response(response: aiohttp.ClientResponse,
                                   display_callback: Optional[Callable[[str], None]] = None) -> str:
    """
