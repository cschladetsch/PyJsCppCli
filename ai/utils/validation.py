...
def validate_input_length(text: str, max_length: Optional[int] = None) -> str:
    ...
    if len(text) > max_length:
        raise ValidationError(
            f"Input text too long. Maximum length is {max_length} characters."
        )
...
def validate_file_path(file_path: Union[str, Path]) -> Path:
    ...
    try:
        ...
        if str(path).startswith(("/etc/", "/sys/", "/proc/", "/dev/")):
            raise FileError(
                f"Access to system files not allowed: {file_path}", str(path)
            )
...
def validate_file_type(
    ...
    if mime_type:
        ...
        if mime_type.startswith("application/x-") and extension not in [
            ".py",
            ".js",
            ".sh",
        ]:
