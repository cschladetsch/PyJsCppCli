...
class CohereClient : public IApiClient {
public:
    explicit CohereClient(const config::ApiConfig& config);
    ~CohereClient() override = default;
...
private:
    ...
    std::string buildChatPayload(const MessageRequest& request) const;
